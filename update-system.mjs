...
/**
 * update-system.mjs — Safe auto-updater for seekiee
 *
...
// Set to true to disable self-update checks (check always reports "offline"
// and never contacts any remote). Enabled now that Seekiee has its own repo.
const UPDATE_CHECK_DISABLED = false;

const CANONICAL_REPO = 'https://github.com/05-deepak-patidar/seekiee-public.git';
const RAW_VERSION_URL = 'https://raw.githubusercontent.com/05-deepak-patidar/seekiee-public/main/VERSION';
const RELEASES_API = 'https://api.github.com/repos/05-deepak-patidar/seekiee-public/releases/latest';
...
async function check() {
  // Honor the kill switch above (set UPDATE_CHECK_DISABLED to skip all remotes).
    ...
    [versionResult, releaseResult] = await Promise.allSettled([
      ...
      fetch(RELEASES_API, {
        headers: {
          ...
          'User-Agent': 'seekiee-update-checker',
        },
