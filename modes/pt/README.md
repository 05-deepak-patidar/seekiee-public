# seekiee -- Modos em Português BR (`modes/pt/`)

Esta pasta contém as traduções em português brasileiro dos principais modos do seekiee para candidatos que buscam vagas no mercado brasileiro ou em empresas que operam em português.
...
O seekiee não tem um "switch de idioma" como flag de código. Em vez disso, existem dois caminhos:
...
| `oferta.md` | `modes/evaluate.md` (ES) | Avaliação completa de uma vaga (Blocos A-F) |
