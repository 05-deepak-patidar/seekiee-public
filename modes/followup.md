...
> "No active applications to follow up on. Apply to some roles first with `/seekiee` and come back when they're aging."
...
- Suggest the user run `/seekiee outreach {company}` to find the right person first
...
> - Trying a different contact via `/seekiee outreach`
...
**To:** {email or "No contact found — run `/seekiee outreach` first"}
