...
Seekiee превращает любой AI-CLI в полноценный командный центр для поиска работы. Вместо ручного ведения таблиц с откликами вы получаете ИИ-пайплайн, который:
...
> **Важно: это НЕ инструмент для массовой рассылки.** Seekiee — это фильтр. Он помогает найти те немногие вакансии, которые стоят вашего времени, из сотен. Система настоятельно рекомендует не откликаться на вакансии с оценкой ниже 4.0/5. Ваше время ценно, как и время рекрутера. Всегда проверяйте перед отправкой.
...
# 6. Начинай
# Вставь URL вакансии или запусти /seekiee
```

## Использование

Seekiee — одна slash-команда с множеством режимов:

```
/seekiee                       → Показать все доступные команды
/seekiee {вставь описание JD}  → Полный авто-пайплайн (оценка + PDF + трекер)
/seekiee scan                  → Сканирование порталов
/seekiee pdf                   → Генерация ATS-резюме
/seekiee batch                 → Пакетная оценка вакансий
/seekiee tracker               → Статус откликов
/seekiee apply                 → Заполнение форм отклика с ИИ
/seekiee pipeline              → Обработка очереди URL
/seekiee outreach              → Сообщение в LinkedIn
/seekiee deep                  → Глубокий анализ компании
/seekiee training              → Оценка курса/сертификации
/seekiee project               → Оценка портфолио-проекта
```

Или просто вставь URL вакансии — seekiee автоматически определит его и запустит полный пайплайн.
...
Seekiee поддерживает ключевые российские джоб-борды:
...
**seekiee — локальный open-source инструмент, НЕ хостинг-сервис.** Используя это ПО, вы подтверждаете:
