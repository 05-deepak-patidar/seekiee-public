#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for seekiee pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node seekiee/verify-pipeline.mjs
 */
...
const SEEKIEE = dirname(fileURLToPath(import.meta.url));
// Support both layouts: data/applications.md (boilerplate) and applications.md (original).
// SEEKIEE_TRACKER overrides the path (used by tests and non-standard layouts).
const APPS_FILE = process.env.SEEKIEE_TRACKER
  ? process.env.SEEKIEE_TRACKER
  : existsSync(join(SEEKIEE, 'data/applications.md'))
    ? join(SEEKIEE, 'data/applications.md')
    : join(SEEKIEE, 'applications.md');
const ADDITIONS_DIR = join(SEEKIEE, 'batch/tracker-additions');
const REPORTS_DIR = join(SEEKIEE, 'reports');
const STATES_FILE = existsSync(join(SEEKIEE, 'templates/states.yml'))
  ? join(SEEKIEE, 'templates/states.yml')
  : join(SEEKIEE, 'states.yml');

// Ensure required directories exist (fresh setup)
mkdirSync(join(SEEKIEE, 'data'), { recursive: true });
...
for (const e of entries) {
  ...
  if (!match) continue;
  const link = match[1];
  if (!existsSync(join(TRACKER_DIR, link)) && !existsSync(join(SEEKIEE, link))) {
