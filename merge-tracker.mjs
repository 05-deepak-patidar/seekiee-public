 ...
 *
 * Run: node seekiee/merge-tracker.mjs [--dry-run] [--verify]
 */
...
if (MIGRATE) {
  ...
  if (DRY_RUN) {
    ...
    console.log(`✅ Migration: rewrote ${changed} report link(s) in ${basename(APPS_FILE)} relative to ${TRACKER_DIR === SEEKIEE ? 'repo root' : 'data/'}`);
  }
  process.exit(0);
}
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(SEEKIEE, 'verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
