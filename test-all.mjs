...
/**
 * test-all.mjs — Comprehensive test suite for seekiee
 *
...
const systemFiles = [
  ...
  '.claude/skills/seekiee/SKILL.md',
];
...
// Add your own personal-data patterns here (full name, phone number,
// private emails, business names) to guard against accidentally committing
// PII into the public repo. Do NOT add the public brand domain
// (darkiee.com) or the public contact address — those are meant to appear.
const leakPatterns = [
];

const scanExtensions = ['md', 'yml', 'html', 'mjs', 'sh', 'go', 'json'];
const allowedFiles = [
  // Files where attribution to the original upstream author is retained
  // for MIT-license compliance.
  'LICENSE', 'NOTICE',
];
...
if (
  ...
  scanMode.includes("don't repeat expensive scraping") &&
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'seekiee-migrate-'));
  try {
    ...
    run(NODE, ['merge-tracker.mjs', '--migrate'], { env: { ...process.env, SEEKIEE_TRACKER: tracker } });
